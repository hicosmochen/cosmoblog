// QUES 如何 Module.export 异步获取的值

const fs = require("fs");
const path = require("path");

function getData() {
  const basePath = `/blogs`;
  const fullPath = path.resolve(__dirname, `..${basePath}`);

  let dirs = fs.readdirSync(`${fullPath}`);
  dirs = dirs.filter((ele) => !ele.startsWith("."));

  const headNav = [];
  const sideBar = {};

  // [A, B, C]
  dirs.forEach((dirName) => {
    const stats = fs.statSync(`${fullPath}/${dirName}`);

    // 是目录
    if (stats.isDirectory()) {
      // 加到顶部主分栏  headNav
      const headNavPath = `${basePath}/${dirName}/`;
      headNav.push({ text: dirName, link: headNavPath });

      const headNacSideBarArray = [];
      let secDirs = fs.readdirSync(`${fullPath}/${dirName}`);
      // console.info("secDirs", secDirs);

      // [C , Java ]
      secDirs.forEach((secDirName) => {
        const secStat = fs.statSync(`${fullPath}/${dirName}/${secDirName}`);
        if (secStat.isDirectory()) {
          let thirdDirs = fs.readdirSync(`${fullPath}/${dirName}/${secDirName}`);
          // console.info("thirdDirs", thirdDirs);
          const children = thirdDirs.map((mdFileName) => [
            `${basePath}/${dirName}/${secDirName}/${mdFileName}`,
            mdFileName.replace(".md", ""),
          ]);
          headNacSideBarArray.push({
            title: secDirName, // 必要的
            // path: `/${dirName}/${secDirName}/`, // 可选的, 标题的跳转链接，应为绝对路径且必须存在
            collapsable: true, // 可选的, 默认值是 true,
            sidebarDepth: 2, // 可选的, 默认值是 1
            children,
          });
          sideBar[headNavPath] = headNacSideBarArray; // 构造下面的结构

          // console.info("headNacSideBarArray", headNacSideBarArray);
        }
      });

      // "/person/": [
      //   {
      //     title: "program", // 必要的
      //     path: "/person/program/", // 可选的, 标题的跳转链接，应为绝对路径且必须存在
      //     collapsable: true, // 可选的, 默认值是 true,
      //     sidebarDepth: 1, // 可选的, 默认值是 1
      //     children: [
      //       ["/person/program/p1.md", "program-p1"],
      //       ["/person/program/p2.md", "program-p2"],
      //     ],
      //   },
      //   {
      //     title: "teacher",
      //     children: [
      //       ["/person/program/p1.md", "program-p1"],
      //       ["/person/program/p2.md", "program-p2"],
      //     ],
      //     initialOpenGroupIndex: -1, // 可选的, 默认值是 0
      //   },
      // ],
    } else {
      console.info(`排除非目录项 ${fullPath}/${dirName}`);
    }
  });

  // console.info("headNav", JSON.parse(JSON.stringify(headNav)));
  // console.info("sideBar", JSON.parse(JSON.stringify(sideBar)));

  return [headNav, sideBar];
}

console.info(path.resolve(__dirname, ".."));
const [headNav, sideBar] = getData();

const finalConfig = {
  title: "Cosmo",
  description: "Cosmo's site",
  base: "/cosmo/",
  themeConfig: {
    nav: headNav,
    sidebar: sideBar,
  },
  plugins: ["vuepress-plugin-table-of-contents"],
};

module.exports = finalConfig;
